...
parts (which we may brush over) are marked with *.  
...
\item (Note that when $X,Y,Z$ are jointly normal, then this definition coincides
  with conditional correlation: \smash{$\rho_{XY|Z} = \Cor(X,Y|Z)$}, but not in
  general)  
...
\item ACF and PACF plots are shown in the top row of Figure
  ...
  suggestive of a nonseasonal MA(2) and a seasonal MA(1), and therefore we try 
  an ARIMA$(0,1,2)(0,1,1)_{12}$ model. Meanwhile, the PACF again shows moderate 
  correlations at lags 1 and 2 and a big spike at lag 12. This suggests a
  nonseasonal AR(2) and a seasonal AR(1), so we can try
  ARIMA$(2,1,0)(1,1,0)_{12}$. Lastly, we can also try the Hyndman-Khandakar
  algorithm, which chooses ARIMA$(1,1,1)(2,1,1)_{12}$     
